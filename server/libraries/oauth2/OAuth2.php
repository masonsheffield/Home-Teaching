...
class OAuth2 {
	 ...
	 */
//	const DEFAULT_ACCESS_TOKEN_LIFETIME = 3600; // one hour
	const DEFAULT_ACCESS_TOKEN_LIFETIME = 15552000; // 6 months
...
//		$methodsUsed = !empty($headers) + isset($_GET[self::TOKEN_PARAM_NAME]) + isset($_POST[self::TOKEN_PARAM_NAME]) + isset($_COOKIE['ultralinkAccessToken']);
//
//		if( $methodsUsed > 1 )
//        {
...
//		}
		...
		if( isset($_POST[self::TOKEN_PARAM_NAME]) )
        {
			...
			if( isset($_SERVER['CONTENT_TYPE']) && ($_SERVER['CONTENT_TYPE'] != 'application/x-www-form-urlencoded') && ($_SERVER['CONTENT_TYPE'] != 'application/x-www-form-urlencoded; charset=UTF-8') )
            {
        ...
        // Cookie method
        if( isset($_COOKIE['ultralinkAccessToken']) )
        {
            return $_COOKIE['ultralinkAccessToken'];
        }
	...
	public function getAuthorizeParams(array $inputData = NULL) {
		...
		if( !isset($inputData) ){ $inputData = $_GET; }
		...
		if ($stored["redirect_uri"] && $input["redirect_uri"] && !$this->validateRedirectUri($input["redirect_uri"], $stored["redirect_uri"])) {
			throw new OAuth2ServerException(self::HTTP_BAD_REQUEST, self::ERROR_REDIRECT_URI_MISMATCH, 'The redirect URI provided (' . $input["redirect_uri"] .') is missing or does not match (' . $stored["redirect_uri"] . ')' );
		}
